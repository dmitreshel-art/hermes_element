...
Примеры:
- пользователь новичок в Element;
- пользователь просит максимально пошаговые инструкции;
- пользователь использует Android;
- пользователь часто спрашивает про recovery key и verification.

### Что НЕ сохранять в user memory
- секреты, ключи, recovery key, пароли, токены;
- содержимое рабочих переписок;
- лишние персональные данные;
- случайные одноразовые детали, которые не помогут в будущем.
...
Память о пользователе — про конкретного человека.
Skills — про общий способ помогать всем пользователям лучше.
